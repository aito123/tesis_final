...
pacman::p_load(here,tidyverse,janitor,glue,haven,rio,sjlabelled,gt,gtsummary,flextable,survey,srvyr,ggpubr,scales,officer, Hmisc, patchwork, ggtext, kableExtra)
...
theme_tesis <- function(x, width_number = "10em") {
  
  x %>% 
  as_kable_extra(booktabs=TRUE, escape = FALSE) %>% #, longtable=T
...
La caída de la PEA ocupada en 2020 también se experimentó en el ámbito urbano como muestra la @tbl-peadomes. Más de 2 millones de empleos se perdieron en el ámbito urbano en 2020. Solo en Lima Metropolitana se perdieron un millón de empleos o el 20% de empleos respecto del 2019 siendo la región más afectada. Se puede observar que la recuperación hacia el 2021 ha sido sostenida, incluso destaca la Selva y el Sur (Costa y Sierra) los cuales han excedido los niveles de empleo prepandemia mostrando una mejor recuperación en líneas generales.

Por otro lado, si bien las ciudades con medio millón de habitantes a más fueron las que más empleos perdieron, alrededor de un millón de empleos, las ciudades intermedias de 50 a 100 mil habitantes fueron las que proporcionalmente perdieron más empleos con cerca de 22% de empleos perdidos en 2020.
...
#   }
#   
#   x[[1]] <- inMinipage(x[[1]], "2.5cm")
#   
#   x %>% 
#   as_kable_extra(booktabs=TRUE, escape = FALSE) %>% #, longtable=T
#   kable_styling(latex_options=c("scale_down", "striped")) %>%  #, "HOLD_position", repeat_header_text="(continuación)"
...
# mutate(dominio=str_wrap(as_label(dominio), width = 20)) %>%
# tbl %>% 
#   add_footnote(label = paste0(strwrap("Lorem ipsum... you know the routine", width=20), collapse = "\n"))

# inMinipage(as_label(base_total$estrato), "2.5cm")
# base_total %>% mutate(estrato=inMinipage(as_label(estrato), "2.5cm")) %>% select(estrato)

# tbl_stack(
#   list(
#     base_total %>% mutate(estrato=inMinipage(as_label(estrato), "2.5cm")) %>% tabla_gen(weight = fac500a, include = dominio, strata = ano),
#     base_total %>% mutate(estrato=inMinipage(as_label(estrato), "2.5cm")) %>% tabla_gen(weight = fac500a, include = estrato, strata = ano, sin_nacional = TRUE)
#   )) %>% theme_tesis() # ya en algun momento retomar
# tbl<-
...
Se observa que, en el 2020, hubo una reducción en 14% de los trabajadores tienen únicamente educación secundaria o menos; no obstante, hacia 2021 se observa una recuperación que llega a superar niveles prepandemia. En cambio, los trabajadores con educación técnica o universitaria completa y más tuvieron una reducción en 20% de este sector con una recuperación mucho más lenta, es decir, que no se equipara a niveles prepandemia. De esta manera, se encuentra que en un mercado laboral compuesto principalmente por trabajadores con educación secundaria o menos, la recuperación de este sector ha tenido mayor celeridad a diferencia de los trabajadores que contaban con un título profesional, condición que no necesariamente les aseguró un puesto de trabajo.
...
Por otro lado, la pandemia ha afectado principalmente a los trabajadores que se consideraban como no pobres en la medida en que hubo una disminución de 2 millones de trabajadores no pobres. Ahora bien, se observa que en 2020 los trabajadores pobres extremos se duplicaron llegando a representar el 1.75% de la PEA ocupada urbana. Hacia el 2021, la presencia de trabajadores pobres extremos no ha cambiado mucho manteniéndose alrededor de 194 mil trabajadores.
...
theme_tesis2 <- function(x, width_number = "10em") {
  
  x %>% 
  as_kable_extra(booktabs=TRUE, escape = FALSE) %>% #, longtable=T
  collapse_rows(1:3) %>% # no funciona el collapse
  kable_styling(latex_options=c("scale_down", "striped", "HOLD_position"), font_size = 20) %>%  # no funciona el font
  column_spec(1, width = width_number) #esto podria ser optativo
...
# tbl1 %>% theme_tesis2(width_number = "16em")
```
...
base_urb %>% 
  tab_esp_cat(weight = fac500a, strata = ano, by = ocupinf, include = estrato, percent = "row", overall=FALSE) %>% theme_tesis(width_number = "16em")
```
...
base_urb %>% 
  ...
  tab_esp_cat(weight = fac500a, strata = ano, by = ocupinf, include = p207, percent = "row", overall=FALSE) %>% theme_tesis(width_number = "6em")
...
base_urb %>% 
  filter(p203 %in% 1) %>% #jefe de hogar
  tab_esp_cat(weight = fac500a, strata = ano, by = ocupinf, include = gedad, percent = "row", overall=FALSE) %>% theme_tesis(width_number = "10em")
```
...
base_urb %>% 
  tab_esp_cat(weight = fac500a, strata = ano, by = ocupinf, include = p507, percent = "row", overall=FALSE) %>% theme_tesis(width_number = "16em")
```

Respecto a la condición de pobreza, se observa que son los trabajadores “no pobres” quienes principalmente aumentaron su presencia en la informalidad entre 2019 y 2021 en un 6% aproximadamente. En el caso de los pobres no extremos y extremos inclusive hubo una ligera disminución de alrededor de 3%.
...
Cabe mencionar que se observa una irregularidad en el caso de la categoría Selva, en la que la diferencia del ingreso promedio mensual a nivel general es negativa (1%); y sin embargo, la diferencia para el caso del empleo formal e informal en este sector es positiva. Este posible error puede deberse a que es una categoría con pocos casos, un error de muestreo, entre otras.
